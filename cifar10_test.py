...
def conv2d(_x, _w, _b):
    return tf.nn.bias_add(tf.nn.conv2d(input=_x, filters=_w, strides=[1, 1, 1, 1], padding='SAME'), _b)


def max_pool(_x, f):
    return tf.nn.max_pool2d(input=_x, ksize=[1, f, f, 1], strides=[1, 1, 1, 1], padding='SAME')
...
def init_w(namespace, shape, wd, stddev, reuse=False):
    with tf.compat.v1.variable_scope(namespace, reuse=reuse):
        initializer = tf.compat.v1.truncated_normal_initializer(
            dtype=tf.float32, stddev=stddev)
        w = tf.compat.v1.get_variable("w", shape=shape, initializer=initializer)

        if wd:
            weight_decay = tf.multiply(
                tf.nn.l2_loss(w), wd, name='weight_loss')
...
def init_b(namespace, shape, reuse=False):
    with tf.compat.v1.variable_scope(namespace, reuse=reuse):
        ...
        b = tf.compat.v1.get_variable("b", shape=shape, initializer=initializer)
    return b


def batch_normal(xs, out_size):
    axis = list(range(len(xs.get_shape()) - 1))
    n_mean, n_var = tf.nn.moments(x=xs, axes=axis)
...
def inference(images, reuse=False):
    ...
    reshape = tf.reshape(
        pool5, [-1, shape[1].value*shape[2].value*shape[3].value])
    ...
    with tf.compat.v1.variable_scope('lw', reuse=tf.compat.v1.AUTO_REUSE):
        with slim.arg_scope([slim.conv2d], normalizer_fn=None, activation_fn=tflearn.relu, padding='SAME'):
            with slim.arg_scope([slim.conv2d, slim.fully_connected],
                                ...
                                weights_regularizer=None, biases_initializer=tf.compat.v1.constant_initializer(0.0)):
...
def train_step(loss, global_step):
    ...
    lr = tf.compat.v1.train.exponential_decay(INITIAL_LEARNING_RATE,
                                    ...
                                    staircase=True)
    tf.compat.v1.add_to_collection('learning_rate', lr)

    train_op = tf.compat.v1.train.MomentumOptimizer(lr, MOMENTUM).minimize(loss)
    return train_op
...
def train():
    with tf.Graph().as_default():
        ...
        test_correct_pred = tf.equal(
            tf.argmax(input=test_logits, axis=1), tf.argmax(input=test_labels, axis=1))
        test_accuracy = tf.reduce_mean(input_tensor=tf.cast(test_correct_pred, tf.float32))
        ...
        for e in range(60):
            ...
            while True:
                try:
                    ...
                    if step % 1000 == 0:
                        lo = sess.run(loss)
                        lr = sess.run(tf.compat.v1.get_collection('learning_rate'))
