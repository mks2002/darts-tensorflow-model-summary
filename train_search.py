...
def main():
	...
	lr=tf.compat.v1.train.cosine_decay(args.learning_rate,global_step,50000/args.batch_size*args.epochs,args.learning_rate_min)

	accuracy=tf.reduce_mean(input_tensor=tf.cast(tf.nn.in_top_k(predictions=logits, targets=y_train, k=1), tf.float32))	
	w_regularization_loss = tf.add_n(utils.get_var(tf.compat.v1.losses.get_regularization_losses(), 'model')[1])
	...
	with tf.control_dependencies([leader_opt,tf.group(*tf.compat.v1.get_collection(tf.compat.v1.GraphKeys.UPDATE_OPS))]):
	...
	valid_accuracy=tf.reduce_mean(input_tensor=tf.cast(tf.nn.in_top_k(predictions=infer_logits, targets=y_valid, k=1), tf.float32))
