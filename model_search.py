...
def MixedOp(x, C_out, stride, index, reduction):
    ...
    with tf.compat.v1.variable_scope(null_scope):
        with tf.compat.v1.variable_scope("arch_var", reuse=tf.compat.v1.AUTO_REUSE):
            weight = tf.compat.v1.get_variable("weight{}_{}".format(2 if reduction else 1, index), [len(
                PRIMITIVES)], initializer=tf.compat.v1.random_normal_initializer(0, 1e-3), regularizer=tf.keras.regularizers.l2(0.5 * (0.0001)))
            ...
            with slim.arg_scope([slim.batch_norm], is_training=is_training):
                ...
                for i in range(layer_num):
                    if i in [layer_num//3, 2*layer_num//3]:
                        ...
                        reduction = False
                    s0, s1 = s1, Cell(
                        s0, s1, cells_num, multiplier, C_curr, reduction, reduction_prev)
                    reduction_prev = reduction
                out = tf.reduce_mean(
                    input_tensor=s1, axis=[1, 2], keepdims=True, name='global_pool')
                logits = slim.conv2d(out, class_num, [
                                     1, 1], activation_fn=None, normalizer_fn=None, weights_regularizer=tf.keras.regularizers.l2(0.5 * (0.0001)))
                logits = tf.squeeze(logits, [1, 2], name='SpatialSqueeze')
    train_loss = tf.reduce_mean(
        input_tensor=tf.nn.sparse_softmax_cross_entropy_with_logits(labels=y, logits=logits))
    ...
    with tf.compat.v1.variable_scope(name, reuse=tf.compat.v1.AUTO_REUSE):
        with slim.arg_scope([slim.conv2d, slim.separable_conv2d], activation_fn=None, padding='SAME', biases_initializer=None, weights_regularizer=tf.keras.regularizers.l2(0.5 * (0.0001))):
            with slim.arg_scope([slim.batch_norm], is_training=is_training):
                ...
                out = tf.reduce_mean(
                    input_tensor=s1, axis=[1, 2], keepdims=True, name='global_pool')
                logits = slim.conv2d(out, 10, [ 1, 1], activation_fn=None, normalizer_fn=None, weights_regularizer=tf.keras.regularizers.l2(0.5 * (0.0001)))
                logits = tf.squeeze(logits, [1, 2], name='SpatialSqueeze')
    train_loss = tf.reduce_mean(
        input_tensor=tf.nn.sparse_softmax_cross_entropy_with_logits(labels=y, logits=logits))
...
def get_genotype(sess, cells_num=4, multiplier=4):
    ...
    def _parse(stride, sess):
        offset = 0
        genotype = []
        arch_var_name, arch_var = utils.get_var(
            tf.compat.v1.trainable_variables(), 'arch_var')
        ...
        for i in range(cells_num):
            ...
            for j in range(i+2):
                ...
                max_index = value_sorted[-2] if value_sorted[-1] == PRIMITIVES.index(
                    'none') else value_sorted[-1]
            ...
            max_edges = [
                edges[np.argsort(edges_confident)[-1]], edges[np.argsort(edges_confident)[-2]]]
